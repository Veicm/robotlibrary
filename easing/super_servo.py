...
from .servo_easing import Servo
...
class Super_Servo():
    ...
    def calibration(self, calibration=None):
        ...
        if calibration:
            self.__servo.calibrate(calibration)
        else:
            return self.__servo.calibrate()
    ...
    def value(self, value=None):
        ...
        if value:
            self.__servo.__angle(value)
        else:
            return self.__servo.__angle()
...
#         remaining_time = self.__duration - self.elapsed_time
        ...
        if self.elapsed_time_in_seconds > self.duration_in_seconds:
            ...
            self.__servo.__angle(self.target_angle)
            sleep(SHORT_SLEEP)
            return True
